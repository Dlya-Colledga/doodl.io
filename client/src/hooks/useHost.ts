...
export interface Player {
	...
	avatar: string;
	score: number;
	isOnline: boolean; // Это поле теперь управляется тайм-аутами сервера
	ping: number;
}
...
export const useHost = () => {
	...
	useEffect(() => {
		...
		// Слушаем ТИК (обновление состояния 20 раз в сек)
		...
		socket.on('game_tick', onGameTick); // Главное событие
